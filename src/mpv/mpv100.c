...
static int load_image16(const char *fn){
    int w,h,c;
    unsigned char *img=stbi_load(fn,&w,&h,&c,3);
...
static int load_palette(const char *fn){
    ...
    if(!strcasecmp(ext,".pal"))    return load_jasc(fn);
...
static void print_help(const char *prog){
    ...
    printf("Supported formats:\n"
           ...
           "  .pal          JASC-PAL ASCII\n"
           "  .gpl          GIMP palette ASCII\n"
           "  .txt          grid or raw-hex (0xRRGGBB / 0xRRGGBBAA)\n"
           "  .aco          Adobe Color Swatch v1\n"
           "  .h            C header (static const uint8_t [256][3])\n"
           "  .png, .pcx    16×16 indexed preview (stb_image)\n");
}
...
int main(int argc,char **argv){
    ...
    glutCreateWindow("MPV - Multi Palette Viewer v1.0.0 by SMR9000");
