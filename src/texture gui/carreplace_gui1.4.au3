...
; — New Extract Textures button —
$Button7 = GUICtrlCreateButton("Extract Textures", 100, 405, 120, 30)
GUICtrlSetFont($Button7, 10, 600, 0, "Segoe UI")
...
While 1
    ...
    Switch $nMsg
        ...
        Case $Button7
            Local $model = GUICtrlRead($Input1)
            If $model = "" Or Not FileExists($model) Then
                MsgBox($MB_ICONERROR, "Error", "Please select a valid CAR model file first.")
                GUICtrlSetData($Edit1, GUICtrlRead($Edit1) & "Error: No CAR file selected for extraction." & @CRLF)
            Else
                GUICtrlSetData($Edit1, GUICtrlRead($Edit1) & "Extracting PNG from: " & $model & @CRLF)
                Run('"' & @ScriptDir & '\car2png.exe" "' & $model & '"', @ScriptDir, @SW_SHOW)
            EndIf
    EndSwitch
WEnd
