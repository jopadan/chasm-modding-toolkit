...
;—————————————— Koda GUI section ——————————————
$Form1    = GUICreate("Chasm CAR Texture Replacer v" & $sVersion & " - GUI Interface by SMR9000", 615, 440, -1, -1, -1, -1)
...
; Info label
$Label6   = GUICtrlCreateLabel("New texture must be same size as original; transparency uses value #040404 in palette.", _
                                0, 385, 615, 20, $SS_CENTER)
...
While 1
    ...
    Switch $nMsg
        ...
        Case $Button1 ; Browse CAR
            $file = FileOpenDialog("Select CAR model file", @ScriptDir, "CAR Files (*.car)", $FD_FILEMUSTEXIST)
            If Not @error Then
        ...
        Case $Button2 ; Browse PNG
            $file = FileOpenDialog("Select PNG texture file", @ScriptDir, "PNG Files (*.png)", $FD_FILEMUSTEXIST)
            If Not @error Then GUICtrlSetData($Input2, $file)

        Case $Button3 ; Browse Palette
            $file = FileOpenDialog("Select ACT palette file", @ScriptDir, "ACT Files (*.act)", $FD_FILEMUSTEXIST)
            If Not @error Then GUICtrlSetData($Input3, $file)

        Case $Button4 ; Browse Output
            $file = FileSaveDialog("Save output CAR file", @ScriptDir, "CAR Files (*.car)", $FD_PATHMUSTEXIST)
            If Not @error Then GUICtrlSetData($Input4, $file)

        Case $Checkbox1 ; Use default palette
        ...
        Case $Button5 ; Process
            ProcessFiles()

        Case $Button6 ; Load original model into viewer
...
Func ProcessFiles()
    ; Clear previous output but keep first line
    ...
    GUICtrlSetData($Edit1, GUICtrlRead($Edit1) & "Process exit code: " & @extended & @CRLF)

    ; Scroll to bottom
    ControlSend($Form1, "", $Edit1, "^{END}")

    ; —————— Auto-launch viewer on new CAR ——————
    If FileExists($outputFile) Then
...
;—————————————— Version info (compiled only) ——————————————
Func _SetVersionInfo()
    Local $sFileVersion    = $sVersion
    Local $sProductVersion = $sVersion
    #AutoIt3Wrapper_Res_Fileversion      = $sFileVersion
    #AutoIt3Wrapper_Res_ProductVersion   = $sProductVersion
    #AutoIt3Wrapper_Res_LegalCopyright   = $sCopyright
    #AutoIt3Wrapper_Res_CompanyName      = $sCompanyName
    #AutoIt3Wrapper_Res_ProductName      = $sProductName
    #AutoIt3Wrapper_Res_FileDescription  = $sDescription
    #AutoIt3Wrapper_Res_Icon_Add         = $sIconPath
    If @Compiled Then
        Local $aVer = FileGetVersion(@ScriptFullPath)
        ConsoleWrite("File version: " & $aVer & @CRLF)
    EndIf
EndFunc
