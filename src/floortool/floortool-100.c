// floors_viewer.c v116 (1.0.0)
// x86_64-w64-mingw32-gcc floors116-FINAL.c -o floortool.exe -I. -I./GL -L./lib -lfreeglut -lopengl32 -lm floortool.res
...
void pasteTileFromClipboard(){
    ...
    glBindTexture(GL_TEXTURE_2D,texMip1[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,32,32,GL_RGBA,GL_UNSIGNED_BYTE,floorMip1Data[ti]);
    glBindTexture(GL_TEXTURE_2D,texMip2[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,16,16,GL_RGBA,GL_UNSIGNED_BYTE,floorMip2Data[ti]);
    glBindTexture(GL_TEXTURE_2D,texMip3[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,8,8,GL_RGBA,GL_UNSIGNED_BYTE,floorMip3Data[ti]);
...
void importGridPNG(){
    ...
    for(int ti=0;ti<MAX_FLOORS;ti++){
      ...
      floorMip2Data[ti]=generateMip(floorMip1Data[ti],w1,h1,&w2,&h2);
      floorMip3Data[ti]=generateMip(floorMip2Data[ti],w2,h2,&w2,&h2);
      glBindTexture(GL_TEXTURE_2D,texBase[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,64,64,GL_RGBA,GL_UNSIGNED_BYTE,floorRGBA[ti]);
      glBindTexture(GL_TEXTURE_2D,texMip1[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,32,32,GL_RGBA,GL_UNSIGNED_BYTE,floorMip1Data[ti]);
      glBindTexture(GL_TEXTURE_2D,texMip2[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,16,16,GL_RGBA,GL_UNSIGNED_BYTE,floorMip2Data[ti]);
      glBindTexture(GL_TEXTURE_2D,texMip3[ti]); glTexSubImage2D(GL_TEXTURE_2D,0,0,0,8,8,GL_RGBA,GL_UNSIGNED_BYTE,floorMip3Data[ti]);
    }
...
void importTilesFromManifest(){
    ...
    while(fgets(line,sizeof(line),mf)){
        ...
        for(int p=0;p<64*64;p++){
            ...
            newRGBA[4*p+0]=R; newRGBA[4*p+1]=G; newRGBA[4*p+2]=B; newRGBA[4*p+3]=255;
            int best=0,bd=INT_MAX;
            for(int c=0;c<256;c++){
                int dr=R-palette[c][0], dg=G-palette[c][1], db=B-palette[c][2];
                int d=dr*dr+dg*dg+db*db;
...
void renderOverlay(){
    ...
    if(showUI){
        ...
        const char *fn=strrchr(g_filename,'/')?strrchr(g_filename,'/')+1:g_filename;
        fn=strrchr(fn,'\\')?strrchr(fn,'\\')+1:fn;
        sprintf(buf,"File: %s",fn);       drawText(buf,10,windowH-20);
        sprintf(buf,"Tiles: %d",MAX_FLOORS);drawText(buf,10,windowH-35);
        sprintf(buf,"Selected: %d",sel.y*8+sel.x);drawText(buf,10,windowH-50);
        sprintf(buf,"Flag: %d",tileFlags[sel.y*8+sel.x]);drawText(buf,10,windowH-65);
        sprintf(buf,"Dither: %s",ditherNames[importDitherMode]);drawText(buf,10,windowH-80);

        const char *ctrls[] = {
          "F1: Toggle UI","Arrow: Select","+/-: Zoom","T: Tile","M: Mipmap",
          "PgUp/Dn: BG","C: Copy","V: Paste","F5: Save grid","F6: Save tiles",
          "F9: Import grid","F10: Import tiles","F12: Save file","R: Reset","DEL: Dither"
        };
        for(int i=0;i<15;i++){
            drawText(ctrls[i],10,windowH-100 - i*15);
        }
    }
    if(dirty){
        drawText("PRESS F12 TO SAVE FILE",10,10);
    }
...
int main(int argc,char **argv){
    g_filename = (argc>1)? argv[1] : "FLOORS.01";
    srand(12345);
    ...
    glutCreateWindow("Chasm The Rift Floor Viewer v1.0.0 by SMR9000");
