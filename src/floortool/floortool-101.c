// floors_viewer.c v118 (1.0.1)
// x86_64-w64-mingw32-gcc floors118-FINAL.c -o floortool.exe -I. -I./GL -L./lib -lfreeglut -lopengl32 -lm floortool.res
...
void renderOverlay(){
    ...
    sprintf(buf,"Selected tile: %d",sel.y*8+sel.x);drawText(buf,10,windowH-50);
    sprintf(buf,"(Not implemented) Flag: %d",tileFlags[sel.y*8+sel.x]); drawText(buf,10,windowH-65);
    
    // Controls (25px gap after File Info)
    drawText("CONTROLS",10,windowH-90);
    const char *ctrls[] = {
        "F1 : Toggle UI","T   : Tile Pattern view","M   :Mipmap view",
        "Arrows : Navigate","+/- : Zoom","ESC : Reset GUI",
        "PgUp/Dn: Change BG color"
    };
    for(int i=0;i<6;i++) drawText(ctrls[i],10,windowH-105 - i*15);
    
    // Import/Export (25px gap after Controls)
    drawText("IMPORT/EXPORT",10,windowH-205);
    const char *io[] = {
        "F5  : Save 512x512 grid to PNG","F6  : Save all 64x64 tiles to folder as PNG","F9  : Import 512x512 grid from PNG",
        "F10 : Import all PNG tiles from folder","F11 : Import selected PNG tile from folder","DEL : Select import dither method"
    };
    for(int i=0;i<6;i++) drawText(io[i],10,windowH-220 - i*15);
    
    // Dither Info (25px gap after Import/Export)
    sprintf(buf,"DITHER METHOD: %s",ditherNames[importDitherMode]);
    drawText(buf,10,windowH-320);
    
    // Tile Operations (25px gap after Dither Info)
    drawText("TILE OPTIONS",10,windowH-345);
    drawText("C : Copy tile",10,windowH-360);
    drawText("V : Paste tile",10,windowH-375);
    drawText("F : Flip selected tile vertically",10,windowH-390);
    drawText("H : Flip selected tile horizontally",10,windowH-405);
    drawText("R : Rotate selected tile 90°",10,windowH-420);
}
...
int main(int argc,char **argv){
    ...
    glutCreateWindow("Chasm The Rift Floor Viewer v1.0.1 by SMR9000");
